...
if( isset($_POST['export_content']) ){
...
}elseif( isset($_POST['download_export_zip']) && current_user_can("manage_options")){
	// Download Created Zip file

	if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'download_export_zip' ) ) {
	    _e( 'Security check Failed. Contact Administrator.', 'vibe' );
	    die();
	}

	$filename = sanitize_file_name($_POST['zip_file']);
	$filepath = $upload_dir_base['basedir'].'/export_demos/'.$filename;
	if ( file_exists($filepath) ) {
		...
		if ( $validate['ext'] != 'zip' ) {
		    die(__("File extension is not allowed.", "wplms"));
		}
		// clean buffer(s)
...
function export_website_content(){
	if ( !isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'],'export_content') || !current_user_can("manage_options")){
	    _e('Security check Failed. Contact Administrator.','vibe');
	    die();
	}

	$default_content = array();
	$post_types = sanitize_text_field($_POST['post_type']);
	$taxonomies = sanitize_text_field($_POST['taxonomy']);
	$options = sanitize_text_field($_POST['option']);
	...
	*/
	...
	if( !empty($_POST['export_title']) ){
		$stylefolder = sanitize_text_field($_POST['export_title']);
	}else{
	...
	foreach ($options as $option) {
		...
		if( $option == 'vibebp_customizer' ){
			$export_vibebp_customizer = 'yes';
		}
		if( $option == 'vibebp_settings' ){
			$export_vibebp_settings = 'yes';
		}
	}
	...
	foreach ( $all_options as $name => $value ) {
		...
		if ( isset($export_vibebp_customizer) && ($export_vibebp_customizer == 'yes') && stristr( $name, 'vibebp_customizer' ) ) {
			$my_options[ $name ] = maybe_unserialize( $value );
		}
		if ( isset($export_vibebp_settings) && ($export_vibebp_settings == 'yes') && stristr( $name, 'vibebp_settings' ) ) {
			$my_options[ $name ] = maybe_unserialize( $value );
		}
	...
	wp_nonce_field("download_export_zip");
	echo '</form>';
