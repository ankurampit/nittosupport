...
function wplms_redirect_to_course($order_id){
    ...
    foreach($items as $item_id => $item){
      ...
      if(isset($order_courses) && is_array($order_courses) && count($order_courses)){
          ...
          if(count($order_courses) == 1 && !empty($order_courses[$item['product_id']]['courses'])  && count($order_courses[$item['product_id']]['courses']) == 1 && ($order->status == 'completed' || $order->status == 'complete')){
