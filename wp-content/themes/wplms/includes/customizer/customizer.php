...
function vibe_customize_scripts(){
    wp_enqueue_script(
        'wplms-customizer-js',
        VIBE_URL . '/includes/customizer/customizer.js',
        array('jquery'),
        rand(0,9999),
        true
    );

    wp_localize_script('wplms-customizer-js', 'WPLMSAdmin', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce'    => wp_create_nonce('wplms_reset_colors_nonce'),
    ));
}
...
function wplms_reset_customizer_colors(){
    ...
    if(empty($option) && is_user_logged_in() && current_user_can('manage_options')){
       die(); 
    }

    if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( wp_unslash( $_POST['security'] ), 'wplms_reset_colors_nonce' ) ) {
