...
if (!empty($products) && in_array($sort, array('price_low', 'price_high', 'popularity', 'latest'), true)) {
    ...
    usort(
        ...
        static function ($a, $b) use ($sort) {
            ...
            /* Popularity */
            if ('popularity' === $sort) {
                return $b->get_total_sales() <=> $a->get_total_sales();
            }
...
<main class="pm-store-page">
    <section class="pm-wrap pm-store-shell">
        <header class="pm-store-hero">
            <h1>Welcome : <span><?php echo esc_html($welcome_name); ?></span> To the Toyo Promotional Items</h1>
        </header>

        <section class="pm-store-content">
                        ...
                        <?php foreach ($products as $product) : ?>
                            <article class="pm-product-card">
                                ...
                                <div class="pm-product-card-body">
                                    <h3><a href="<?php echo esc_url(get_permalink($product->get_id())); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html($product->get_name()); ?></a></h3>
            ...
            <aside class="pm-store-sidebar" aria-label="Product categories">
                ...
                <div class="pm-side-box pm-special-box">
                    <h3>Specials</h3>
                    <p>Seasonal items and limited offers appear automatically when products are marked as featured or on sale.</p>
                </div>
            </aside>
        </section>
    </section>
