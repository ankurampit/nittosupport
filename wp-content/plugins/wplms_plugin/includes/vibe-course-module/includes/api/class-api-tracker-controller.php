...
if ( ! class_exists( 'BP_Course_Rest_Tracker_Controller' ) ) {
	...
	class BP_Course_Rest_Tracker_Controller{
	    ...
	    function __construct(){
			...
			/* Asssignemnnts */
			/*
			if(in_array($post->post_type,array('wplms-assignment'))){
	    		if(empty($this->tracker['assignments'])){
		    		$this->tracker['assignments'] = array($post_id=>time());
		    	}else if(!in_array($post_id,$this->tracker['assignments'])){
		    		$this->tracker['assignments'][$post_id]=time();
		    	}
	    	}
	    	*/
	    	add_action('wplms_start_assignment',array($this,'reload_assignment'),10,2);
	    	add_action('wplms_submit_assignment',array($this,'reload_assignment'),10,2);
	    	add_action('wplms_assignment_reset',array($this,'reload_assignment'),10,2);
	    	add_action('wplms_evaluate_assignment',array($this,'reload_myassignment'),10,3);
			...
			add_action( 'xprofile_updated_profile', array($this,'wplms_api_user_profile_update'), 10, 5 );
	    ...
	    function update_user_tracker($user_id){
	    	...
	    	if(empty($this->user_tracker['counter'])){
	    		$this->user_tracker = array_merge($this->user_tracker,array('counter'=>0));
	    	}
		...
		function fetch_tracker($timestamp,$user_id=null){
			...
			if(!empty($tracker)){
				...
				foreach($tracker as $key => $item){
					...
					if(is_array($tracker[$key])){
						$newTracker[$key]=array();
						foreach ($tracker[$key] as $k=> $v) {
							if( $v <= $timestamp){
								unset($tracker[$key][$k]);
							}
						}
						$newTracker[$key]=$tracker[$key];
					}else{
						if( $item > $timestamp){
							$newTracker[$key]=$item;
						}
		...
		*/
		...
		function reload_assignment($assignment_id,$user_id){
	    	...
	    	if(empty($this->user_tracker['assignments'])){
	    		$this->user_tracker['assignments'] = array($assignment_id=>time());
	    	}else{
	    		$this->user_tracker['assignments'][$assignment_id] = time();
	    	}
	    	
	    	$this->update_user_tracker($user_id);
    	}

    	function reload_myassignment($assignment_id,$marks,$user_id){
	    	$this->reload_assignment($assignment_id,$user_id);
    	}


    	function wplms_api_user_profile_update( $user_id, $field_ids = array(), $errors = false, $old_values = array(), $new_values = array() ) {
   
    		$this->get_tracker();	
			if(empty($this->tracker['members']))	{
				$this->tracker['members'] = array();
			}
			$this->tracker['members'][$user_id] = time();
			$this->update_tracker();
    	}
