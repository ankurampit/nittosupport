...
if ( ! class_exists( 'BP_Course_Rest_Course_Controller' ) ) {
	...
	class BP_Course_Rest_Course_Controller extends BP_Course_Rest_Controller {
		...
		function get_instructor_course($request){
			...
			$instructor_id = $this->set_current_user_id_by_token();
			$filter = $post->filter;
			$courses = array();
			$query_args = apply_filters('query_args_get_instructor_course',array(
				'post_type'=>'course',
			...
			$data = apply_filters('vibe_get_instructor_course',$data,$user_id);
	    	return new WP_REST_Response($data, 200);
		}
