...
if ( ! class_exists( 'BP_Course_New_Rest_User_Controller' ) ) {
	...
	class BP_Course_New_Rest_User_Controller extends BP_Course_New_Rest_Controller {
		...
		function deletemediaupload($request){
	    	...
	    	$data = array('status'=>false,"message"=>"Some error");

	    	if(!empty($args['media'])  && !empty($args['media']['id'])){
	    		$author = get_post_field('post_author',$args['media']['id']);
	    		if($this->user_id == $author || user_can($this->user_id,"manage_options") && str_contains( $args['media']['file'],'wp-content')){
	    			wp_delete_file( $args['media']['file'] );	
	    			$data = array('status'=>true,'media'=>$args['media']);
	    		}
	    	}
			...
			if(!empty($activity_id)){
				...
				if(!empty($body['course'])){
					...
					if($drip_check['status']){
						...
						$quiz_data = apply_filters('wplms_before_bp_wplms_get_quiz_data',false,$quiz_id,$user_id,$body['course']);
						if(empty($quiz_data )){
		...
		function add_review($request){
			...
			$data = apply_filters('wplms_api_add_review_args',array(
			...
			if(strlen($review) < apply_filters('wplms_add_review_content_length',10)){
				$status = 0;
		...
		*/
		...
		function get_transactions($request){
			...
			$page = intval($request['paged']); 
			if(empty($request['paged'])){$page =1;}
			if(empty($request['action'])){$type = '';}else{
				$type = 'AND a.type = "'.esc_sql($wpdb->esc_like($request['type'])).'"';
			}
       ...
       function get_comment_child($comment_id)
       {
	    ...
	    function search_users_in_chat($request){
	    	global $wpdb;
	    	$user_initials = esc_sql($wpdb->esc_like($request->get_param('user_initials')));
		...
		function fetch_media($request){
			...
			if(!empty($post['posts_per_page'])){
				if($post['posts_per_page']<=20){
					$posts_per_page = intval($post['posts_per_page']);
				}
			}
            ...
            $query =  array(
                ...
                'post_status' => 'any',
                'post_mime_type' => !empty($post['post_mime_type'])?sanitize_text_field($post['post_mime_type']):"",
                'posts_per_page' => $posts_per_page,
                'paged' => !empty($post['paged'])?intval($post['paged']):1,
                'orderby' => !empty($post['order'])?sanitize_text_field($post['order']):'',
                's' => !empty($post['search_terms'])?esc_sql($wpdb->esc_like($post['search_terms'])):''
            ) ;
            ...
            if ( !empty( $query['s'] ) ) {
                global $wpdb;
                $post_in = [];
                $posts = $wpdb->get_results("SELECT ID from {$wpdb->posts} WHERE guid LIKE '%" . $wpdb->esc_like($query['s']) . "%' AND post_status='inherit'");
                if(!empty($posts)){
