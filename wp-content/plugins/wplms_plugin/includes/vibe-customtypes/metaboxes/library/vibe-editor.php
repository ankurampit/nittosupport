...
function custom_post_carousel($atts, $content = null) {
    ...
    if(empty($attributes['inline_styles'])){$attributes['inline_styles']='';}
    if(empty($atts['post_type'])){$atts['post_type']='post';}
    if(empty($atts['carousel_number'])){$atts['carousel_number']=4;}
    if(empty($atts['column_width'])){$atts['column_width']=320;}
    if(empty($atts['featured_style'])){$atts['featured_style']='';}
  ...
  if(!isset($atts['post_ids']) || strlen($atts['post_ids']) < 2){
        ...
        $more= '';
        if(isset($atts['show_more']) && $atts['show_more'] && !empty($atts['more_link'])) {
        ...
        $output .= '
          <style>
          #'.$rand.' ul.slides>div>div>li{padding:0 .75rem;}
          #'.$rand.' ul.slides>div>div{margin:0 -0.75rem;}
          #'.$rand.' ul.slides .courseitem{margin:1px;}
         </style>
        ';
        $min = 1;$max=4;
        ...
        if(!empty($atts['carousel_rows']) && $atts['carousel_rows'] > 1 && $row_logic<$atts['carousel_number']){
          ...
          $row_logic ++;
            ...
            $length = 20;
            if(!empty($atts['carousel_excerpt_length'])){
                $length = $atts['carousel_excerpt_length'];
            }
            
           $output .= '<li>';
          $output .= thumbnail_generator($post,$atts['featured_style'],$cols,$length );  
          $output .= '</li>';
        }
  ...
  function wplms_custom_post_carousel($atts, $content = null) {
    ...
    function taxonomy_carousel($atts,$content=null){
        ...
        $output .= '
          ...
          #'.$rand.' ul.slides div>li{padding:0 0.5rem;}
         </style>
        ';
        $min = 1;$max=4;
        if(!empty($atts['carousel_min'])){
    ...
    function member_carousel($atts,$content=null){
        ...
        $output .= '
          ...
          #'.$rand.' ul.slides div>li{margin:0 0.5rem;}
         </style>
        ';
        $min = 1;$max=4;
        if(!empty($atts['carousel_min'])){
        ...
        $output .='</style><div id="member_grid'.$rand.'">
            ...
            if(!empty($user_ids)){
                ...
                if(empty($atts['column_width'])){
                    $atts['column_width'] = 150;
                }
                if(empty($atts['column_width'])){
                    $atts['carousel_link'] = false;
                }
                if(empty($atts['carousel_link'])){
                    $atts['carousel_link'] = false;
                }
    ...
    function custom_post_filterable($atts, $content = null) {
       ...
       $output .= "<div {$attributes['class']}{$attributes['inline_styles']}>";
        ...
        if($atts['post_type'] == 'course' && !empty($atts['course_style'])){
            switch($atts['course_style']){
                case 'popular':
                  $query_args['orderby'] = 'meta_value_num';
                  $query_args['meta_key'] = 'vibe_students';
        ...
        foreach($cats as $cat){
          if(!in_array($cat->slug,$exclude_terms)){
            ...
            if(!empty($atts['taxonomy']) && $atts['taxonomy'] == 'course-cat'){
...
function vibe_custom_slider($atts, $content) {
	...
	if(!isset($atts['post_ids']) || strlen($atts['post_ids']) < 2){
        ...
        if(!empty($atts['post_type']) && $atts['post_type'] == 'course' && !empty($atts['course_style'])){
            switch($atts['course_style']){
                case 'popular':
                  $query_args['orderby'] = 'meta_value_num';
                  $query_args['meta_key'] = 'vibe_students';
  	 ...
  	 if(empty($masonry)){
        ...
        $grid_link = 0;
        if(!empty($atts['grid_link'])){
            $grid_link = $atts['grid_link'];
        }
        $featured_style= 'course2';
        if(!empty($atts['featured_style'])){
            $featured_style=$atts['featured_style'];
        }

        $output .= '<li class="'.$atts['grid_columns'].'" '.$style.'>';
        $output .= thumbnail_generator($post,$featured_style,$cols,$atts['grid_excerpt_length'],$grid_link);
        $output .= '</li>';
