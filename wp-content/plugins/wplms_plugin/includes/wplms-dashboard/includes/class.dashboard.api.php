...
class WPLMS_Dashboard_Api
{
    ...
    function new_instructor_announcement($request)
    {
        ...
        $announcement = nl2br($body['announcement']['message']);
        $course_link = get_the_permalink($course_id);
    ...
    function get_course_progress($request)
    {
        ...
        if(!empty($body['course_id'])){
            $_course_id = intval($body['course_id']);
        }
        $num = 5;
    ...
    function get_contact_user($request){
        ...
        if(!empty($user_id) && !empty($type)){
            switch ($type) {
                case 'get_friends':
                    $data = array('status' => 1,'data'=>$this->get_friends($user_id,$s));
                break;
                case 'get_course_students':
                    $data = array('status' => 1,'data'=>$this->get_course_students($user_id,$s));
                break;
                case 'get_instructors':
                    $data = array('status' => 1,'data'=>$this->get_searched_users('instructor',$s));
                break;
                case 'get_admins':
                    $data = array('status' => 1,'data'=>$this->get_searched_users('administrator',$s));
                break;  
                default:
                    $data = array('status' => 0,'message'=>__('Type not matched','wplms') );
                break;
    ...
    function get_course_students($user_id,$s=null){
        global $wpdb;
        $query = apply_filters('wplms_dashboard_courses_instructors',$wpdb->prepare("
                SELECT posts.ID as course_id
                FROM {$wpdb->posts} AS posts
        ...
        if ( isset($course_students) && is_array( $course_students) ) {
            ...
            $query ="
              ...
              WHERE (u.user_login LIKE '{$search_term}' OR u.user_nicename LIKE '{$search_term}' OR u.user_email LIKE '{$search_term}' OR u.display_name LIKE '{$search_term}')
            AND ID IN ({$user_ids})
              LIMIT 0, 9999999
            ";
    ...
    function get_searched_users($role ,$s=null){
        global $wpdb;
        $role  = '%"' . $wpdb->esc_like($role ) . '"%';
        //$role  = '%"' . $wpdb->esc_like('administrator') . '"%';
        // Search term sanitization
        $search_term = '%' . $wpdb->esc_like($s) . '%';

        // Custom query for users with 'Instructor' role and matching search criteria
        $query ="
          SELECT u.ID
          FROM {$wpdb->users} u
          INNER JOIN {$wpdb->usermeta} um ON u.ID = um.user_id
          WHERE um.meta_key = '{$wpdb->prefix}capabilities'
          AND (um.meta_value LIKE '{$role}')
          AND (u.user_login LIKE '{$search_term}' OR u.user_nicename LIKE '{$search_term}' OR u.user_email LIKE '{$search_term}' OR u.display_name LIKE '{$search_term}')
    ...
    function get_friends($user_id,$s=null){
