...
class wplms_dash_calendar extends WP_Widget {
    ...
    $widget_ops = array( 'classname' => 'wplms_dash_calendar', 'description' => __('Recent activity from Student', 'wplms') );
    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wplms_dash_calendar' );
    parent::__construct( 'wplms_dash_calendar', __(' DASHBOARD : Calendar', 'wplms'), $widget_ops, $control_ops );
  }
    ...
    if ( $title )
		...
		if(isset($activities) && is_array($activities)){
			...
			echo '<div class="student_activity"><a class="small button '.(( $title )?'withtitle':'').'">'.__('VIEW ALL','wplms').'</a>
			...
			foreach($activities as $activity){
				...
				echo '<li class="'.$activity->component.' '.$activity->type.'">
						<div class="dash-activity">
							<span class="dash-activity-time">'.$time.' '.__('AGO','wplms').'</span>
    ...
    function form($instance) {  
        ...
        <p>
          <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of activities in one screen','wplms'); ?></label> 
        ...
        <p>
          <label for="<?php echo $this->get_field_id('friends'); ?>"><?php _e('Show Me & My Friends Activity','wplms'); ?></label> 
