...
class VibeBP_API_Init{
    ...
    function get_member_attachments($request){
        ...
        if(empty($list)){
            $data = array(
                'status' => 1,
                'message' => _x('No Media Found','No Media Found','vibebp'),
                'data' => $list,
                '$query'=>$query
            );
        }else{
            $data = array(
                'status' => 1,
                'message' => _x('Media Found','Media Found','vibebp'),
                'data' => $list,
        ...
        if(!empty($_FILES['file'])){
            ...
            if($can_upload){
                ...
                if ( $movefile && ! isset( $movefile['error'] ) ) {
                    if ( $movefile && !isset( $movefile['error'] ) ) {
                        ...
                        if(!empty($attach_id)){
                            ...
                            if($post){
                                ...
                                $return = array('status'=> 1,'message'=>__('File is valid, and was successfully uploaded','vibebp'),'data'=>$attachment_data);
                            }
                        }
                    }
                } else {
                     ...
                     */
                    $return=array('status'=> 0,'message'=>__('File uploading failed','vibebp'),'data'=>$movefile);
                }   
            }else{
                $return=array('status'=> 0,'message'=>__('Can not upload','vibebp'),'data'=>[]);
            }
        }else{
            $return=array('status'=> 0,'message'=>__('File not found','vibebp'),'data'=>[]);
        }
        return new WP_REST_Response($return, 200);
    }
    ...
    function forgotPassword($request){
        ...
        if(empty($body['email']) || (defined('FILTER_VALIDATE_EMAIL') && !filter_var($body['email'], FILTER_VALIDATE_EMAIL))){
        ...
        if ( function_exists('bp_send_email') && !$check){
            ...
            return new WP_REST_Response(array('status'=>1,'message'=>_x('Please check your email for password recovery !','forgot password mail message','vibebp'),"output"=>$check),200);
        }
    ...
    function getCurrentComponentTours($request){
        ...
        if($query->have_posts()){
            ...
            $return = array( 'status' => 0, 'message'=>_x('No tours found','api message','vibebp') );
        }
    ...
    function selectcpt($request){
        ...
        if($query->have_posts()){
            ...
            $return = array('status'=>false,"check"=>"True",'message'=>_x('Sorry Something went wrong or invalid post type','some error','vibebp'));
        }
        wp_reset_postdata();
